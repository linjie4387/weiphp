...
class CustomMenuController extends AddonsController {
	...
	function get_data($map) {
		$map ['token'] = get_token ();
		//$sql = M ( 'custom_menu' )->where ( $map )->fetchSql(true)->select();
	...
	function send_menu() {
		...
		if($group_id){
			$tree2['matchrule'] = array("group_id"=>$group_id);
		}
		$tree = $this->json_encode_cn ( $tree2 );
		$access_token = get_access_token ();
		if($group_id){//按分组个性化菜单
			//file_get_contents ( 'https://api.weixin.qq.com/cgi-bin/menu/delconditional?access_token=' . $access_token );
