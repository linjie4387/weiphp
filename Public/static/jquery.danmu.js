		...
		callback: function( index ) {
			...
			if($(element).data("danmu_array")[$(element).data("nowtime")]){
				...
				for(var i=0;i<danmus.length;i++){
					var a_danmu="<div class='flying flying2 c_item' id='linshi'><span class='img_wrap'><img src=''/></span><span class='content'></span></div>";
					$(element).append(a_danmu);
					$("#linshi .content").text(danmus[i].text).css('color',danmus[i].color);
					$("#linshi img").attr('src',danmus[i].picUrl);
					$("#linshi .img_wrap").css('border-color',danmus[i].color);
					if (danmus[i].hasOwnProperty('isnew')){
						$("#linshi .content").text(danmus[i].text).css('color',danmus[i].color);
						$("#linshi").addClass('is_new');
					}
