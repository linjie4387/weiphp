...
function(a, b, c) {
	...
	b.module("active", ["services.active"]), b.module("active").controller("ActiveCtrl", ["$rootScope", "$http", "$scope", "$timeout", "security", "$window", "activeService", "MineService", "$routeParams", function(a, b, c, d, e, f, g, h, i) {
		...
		d.theme = "bootstrap", a.when("/home", {
			feedBackUrl: "http://e.wesambo.com/s/vk2Ucz15",
			...
			feedBackUrl: "http://e.wesambo.com/s/2qFiYzrc",
			...
			feedBackUrl: "http://e.wesambo.com/s/euMcJX7J",
			...
			feedBackUrl: "http://e.wesambo.com/s/zJuTC6jT",
			...
			feedBackUrl: "http://e.wesambo.com/s/QblumX2x",
			...
			feedBackUrl: "http://e.wesambo.com/s/eWdpV5XU",
		...
		a.toPage = 1, a.model = {}, a.downLoad = function(a, b) {
			var c;
			c = JSON_URL + "?c=custom&a=exp" + (j ? "&user=" + j : ""), b && (c += (/\?/.test(c) ? "&" : "?") + "origin=" + b), a && (c += (/\?/.test(c) ? "&" : "?") + "groupId=" + a), location.href = c
		}, a.staticFileds = [{
			id: "name",
			name: "姓名"
		}, {
		...
		g.dataOutNoHeader = function() {
			...
			var c;
			c = C + (/\?/.test(C) ? "&" : "?") + "start=" + a + "&end=" + b, location.href = c, g.dataPageList = !1
		};
		var D = function(a, b) {
		...
		f.scene || (f.scene = {}), f.editScene = function() {
			...
			h.getSceneDetail(f.sceneId, v).then(function(a) {
				f.scene = a.data.obj, f.scene.applyPromotion = "" + f.scene.applyPromotion, f.scene.applyTemplate = "" + f.scene.applyTemplate, f.scene.isTpl = "" + f.scene.isTpl, 2 == a.data.obj.pageMode && (a.data.obj.pageMode = 0), f.scene.property ? f.scene.property = JSON.parse(f.scene.property) : f.scene.property = {}, f.code = PREFIX_URL + "eqs/qrcode/" + f.scene.code + ".png", f.url = PREFIX_HOST + "v-" + f.scene.code, f.customUrl = l.trustAsResourceUrl(PREFIX_HOST + "index.php?c=view&id=" + f.scene.id + "&preview=preview"), v && (f.customUrl += (/\?/.test(url) ? "&" : "?") + "user=" + v), f.scene.property.autoFlip && b.forEach(f.autoFlipTimes, function(a, b) {
		...
		return function(d) {
			return d.then(null, function(e) {
				if (401 === e.status) {
					if ("/home" == b.path() || "/home/login" == b.path() || "/home/register" == b.path() || "/home/reset" == b.path() || "/agreement" == b.path() || "/reg" == b.path() || "/sample" == b.path() || "/error" == b.path() || 0 === b.path().indexOf("/active")) return;
		...
		function f(c) {
			...
			c && (d += "&bizType=" + c), d += (/\?/.test(d) ? "&" : "?") + "time=" + (new Date).getTime(), b({
				withCredentials: !0,
				method: "GET",
				url: JSON_URL + d
			}).then(function(b) {
