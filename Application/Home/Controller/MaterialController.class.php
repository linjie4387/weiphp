...
class MaterialController extends HomeController {
	...
	function material_lists() {
		...
		foreach ( $list ['list_data'] as &$vo ) {
			...
			$sql = M ( 'material_news' )->field ( "id,title,cover_id,intro,group_id" )->where ( $map2 )->order("id")->select (false);
			error_log("sql:".$sql );
			$vo ['child'] = M ( 'material_news' )->field ( "id,title,cover_id,intro,group_id" )->where ( $map2 )->order("id")->select ();
		}
		$this->assign ( $list );
